...
export const JefePasantiasPanel: React.FC = () => {
  ...
  const cargarPasantias = async () => {
    ...
    try {
      ...
      setAviso({
        tipo: 'error',
        mensaje:
          error.message || 'No se pudieron cargar las pasantías asignadas.',
      });
    } finally {
      setCargando(false);
    }
  ...
  const abrirCrearActividad = (fecha: string) => {
    ...
    if (actividadesDelDia.length >= MAX_ACTIVIDADES_DIA) {
      setAviso({
        tipo: 'warning',
        mensaje:
          'Este día ya tiene 5 actividades marcadas. Edita una existente o elige otra fecha para no sobrecargar el calendario.',
      });
      return;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-medium-gray">
                Cuando el gerente te asigne una pasantía, podrás crear y administrar actividades desde esta sección.
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-medium-gray">
            Selecciona una pasantía y registra actividades asignadas a pasantes aprobados.
          </p>
        </div>
      </div>
          ...
          {pasantias.map((pasantia) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left bg-white-main p-5 rounded-xl shadow-sm border transition-all ${
                  activa
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      <span className="text-[11px] font-bold bg-main-green/10 text-main-green px-2 py-1 rounded-md">
                        {pasantia.pasantes_aprobados_count || 0} pasantes
                      </span>

                      <span className="text-[11px] font-bold bg-institucional-blue/10 text-institucional-blue px-2 py-1 rounded-md">
                        {(pasantia.actividades || []).length} actividades
                      </span>
                    </div>
                  </div>
                </div>
    ...
    <div className="bg-white-main p-6 rounded-xl shadow-sm border border-light-gray">
      <div className="flex items-start gap-3">
        ...
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-dark-gray mt-5">
            <DatoResumen icono={<Calendar size={16} />} label="Inicio" valor={pasantia.fecha_inicio} />
            <DatoResumen icono={<Calendar size={16} />} label="Fin" valor={pasantia.fecha_fin} />
            <DatoResumen icono={<Clock size={16} />} label="Horario" valor={pasantia.horario} />
            <DatoResumen icono={<UserCircle size={16} />} label="Pasantes" valor={`${pasantia.pasantes_aprobados_count || 0}`} />
          </div>
        </div>
      </div>
    </div>
...
const DatoResumen: React.FC<DatoResumenProps> = ({ icono, label, valor }) => {
  return (
    <div className="bg-light-gray/40 rounded-xl px-4 py-3">
      ...
      <p className="font-bold text-dark-gray mt-1">{valor || 'No definido'}</p>
    </div>
  );
};
    ...
    <div className="bg-white-main p-6 rounded-xl shadow-sm border border-light-gray">
      <div className="flex items-center justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-medium-gray">
            Haz clic en una fecha para agregar una actividad. Máximo {MAX_ACTIVIDADES_DIA} actividades por día.
          </p>
        </div>
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {diasSemana.map((dia) => (
          <div key={dia} className="text-center text-xs font-bold text-medium-gray uppercase">
            {dia}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {dias.map((fecha) => {
          ...
          const lleno = actividadesDelDia.length >= MAX_ACTIVIDADES_DIA;
          ...
          return (
            <button
              ...
              className={`min-h-36 border rounded-xl p-2 text-left transition-colors ${
                actividadesDelDia.length > 0
                  ? obtenerClaseDiaConActividad(actividadesDelDia[0].color)
                  : 'bg-light-gray/30 hover:bg-main-green/10 border-light-gray'
                ...
                <span className="flex items-center gap-1">
                  {actividadesDelDia.length > 0 && (
                    <span className="text-[10px] font-bold text-medium-gray">
                      {actividadesDelDia.length}/{MAX_ACTIVIDADES_DIA}
                    </span>
                  )}
                  <Plus size={14} className={lleno ? 'text-medium-gray' : 'text-main-green'} />
                </span>
              </div>

              <div className="mt-2 space-y-1">
                {actividadesDelDia.slice(0, MAX_ACTIVIDADES_DIA).map((actividad) => (
                  <div
                    key={actividad.id_actividad}
                    className={`border rounded-lg px-2 py-1 shadow-sm ${obtenerClaseActividad(
                      actividad.color
                    )}`}
                  ...
                  >
                    ...
                    <p className="text-[10px] opacity-80 line-clamp-1">
                      {obtenerNombrePasanteActividad(actividad)}
                    </p>

                    <EstadoMini estado={actividad.estado || 'pendiente'} />
...
const EstadoMini: React.FC<{ estado: string }> = ({ estado }) => {
  const texto = estado === 'completada'
    ? 'Completada'
    : estado === 'en_progreso'
    ? 'En progreso'
    : 'Pendiente';
  ...
  return (
    <span className="inline-block text-[9px] font-bold opacity-80 mt-1">
      {texto}
    </span>
  );
};

const obtenerNombrePasanteActividad = (actividad: ActividadJefe) => {
  const usuario = actividad.pasante?.usuario;

  if (!actividad.id_pasante || !usuario) return 'Sin pasante asignado';

  return `${usuario.nombre || ''} ${usuario.apellido || ''}`.trim() || 'Pasante asignado';
};
...
const obtenerClaseActividad = (color?: string) => {
  ...
  return clases[color || 'verde'] || clases.verde;
};

const obtenerClaseDiaConActividad = (color?: string) => {
  const clases: Record<string, string> = {
    verde: 'bg-main-green/10 border-main-green/30',
    azul: 'bg-institucional-blue/10 border-institucional-blue/30',
    morado: 'bg-purple-50 border-purple-200',
...
const AvisoTailwind: React.FC<{ aviso: Aviso }> = ({ aviso }) => {
  ...
  return (
    <div className={`border px-4 py-3 rounded-xl text-sm flex gap-2 items-start ${estilos[aviso.tipo]}`}>
